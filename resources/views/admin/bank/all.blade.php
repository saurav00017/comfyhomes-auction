  ...
  <div class="row">
    <div class="col-12" >
      <div class="card">
        ...
        <a href="{{config('app.baseURL')}}/admin/create-bank"> <button type="submit" class="btn pull-right btn btn-primary panel-button add-btn" >Create Bank</button></a><br>
...
$(document).ready( function () {
    var table = $('#data-table').DataTable({
        ...
        serverSide: true,
        order: [[0, 'desc']],
        ajax: "{{config('app.baseURL')}}/admin/bank-management-allData",
        columns: [
        
 {
        ...
        {
                'mRender':function(data,type,row){
                return "<img src={{config('app.baseURL')}}/storage/app/"+ row.icon +" style=width:50%>";
            }
          },      
        ...
        {"targets":-1,
             ...
             "mRender": function(data, type, row) {
             if(row.is_active==0){
             return "<a class=datatable-left-link href={{config('app.baseURL')}}/admin/active-bank/" + row.id+"><span><button type='submit' class='btn btn-success'>Active</button></span></a>";
              
            }
            else{
               return "<a class=datatable-left-link href={{config('app.baseURL')}}/admin/inactive-bank/" + row.id+"><span><button type='submit' class='btn btn-danger'>InActive</button></span></a>";
               }
            },
          },
